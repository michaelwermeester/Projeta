...
- (NSManagedObjectModel *)managedObjectModel
{
	...
	if (!managedObjectModel) {
		...
		if (!managedObjectModel) {
			NSLog(@"%@:%@ unable to load model at URL %@", [self class], NSStringFromSelector(_cmd), modelURL);
			return nil;
		}
		...
		cachedModel = managedObjectModel;
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    if (![persistentStoreCoordinator addPersistentStoreWithType:YOUR_STORE_TYPE configuration:nil URL:storeURL options:nil error:&error]) {
